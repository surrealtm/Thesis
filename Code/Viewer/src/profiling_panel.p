...
draw_profiling :: (*void, element: *UI_Element, data: *Profiling_UI_Data) {
    ...
    for i := 0; i < viewer.profiling_data.timeline_count; ++i {
        ...
        bg_color: GFX_Color = .{ entry.depth * 16, 100, 255, 255 };
    ...
    //
    {
        marker_color :: GFX_Color.{ 200, 200, 200, 255 };
        gfx_draw_line(gfx, .{ x0, element.screen_position.y }, .{ x0, element.screen_position.y + element.screen_size.y }, 2, marker_color); // Mark the start of profiling
        gfx_draw_line(gfx, .{ x0 + w, element.screen_position.y }, .{ x0 +  w, element.screen_position.y + element.screen_size.y }, 2, marker_color); // Mark the end of profiling
    }
...
update_profiling :: (input: UI_Input, element: *UI_Element, data: *Profiling_UI_Data) {
    ...
    if data.viewer.window.button_pressed[.Right] {
        data.zoom   = .{ 1, 1 };
        data.center = .{ .5, .5 };
    }
}
...
profiling_ui :: (viewer: *Viewer) {
    ui_push_window(*viewer.ui, "Profiling", .{ .5, .5 }, .Draggable);
    ui_set_width(*viewer.ui, .Pixels, 1000, 1);
    ui_set_height(*viewer.ui, .Pixels, 500, 1);
